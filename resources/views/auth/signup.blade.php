...
@section('content')
    {{-- REMOVED: Wrapper div. Card uses w-full to fill the layout container --}}
    <x-card class="w-full p-2">
        ...
        <x-card.content>
            ...
            <form id="signup-form" action="{{ route('signup.post') }}" method="POST" class="space-y-5">
                ...
                {{-- RESPONSIVE FIX: Stack names on mobile (flex-col), side-by-side on desktop (md:flex-row) --}}
                ...
                <div>
                    ...
                    <x-input id="email" name="email" type="email" placeholder="someone@gmail.com"
                        autocomplete="email" autocapitalize="none" class="w-full" />
                </div>
                ...
                <div>
                    ...
                    <x-input id="password_confirmation" name="password_confirmation" type="password"
                        placeholder="••••••••" autocomplete="new-password" class="w-full" />
                </div>

                <div>
                    <x-label for="pin">Create PIN (6 digits)</x-label>
                    <x-input id="pin" name="pin" type="password" inputmode="numeric" pattern="\d*"
                        maxlength="6" placeholder="••••••" autocomplete="new-password" class="w-full" />
                </div>
                ...
                <div>
                    ...
                    <x-input id="pin_confirmation" name="pin_confirmation" type="password" inputmode="numeric"
                        pattern="\d*" maxlength="6" placeholder="••••••" autocomplete="new-password" class="w-full" />
                </div>
            </form>
        </x-card.content>
        ...
        <x-card.footer class="flex flex-col gap-y-5">
            ...
            {{-- Sign In --}}
            <x-card.description>Already have an account? <x-link class="text-primary" href="{{ route('login') }}">Sign in</x-link>
            </x-card.description>
        </x-card.footer>
    </x-card>
@endsection